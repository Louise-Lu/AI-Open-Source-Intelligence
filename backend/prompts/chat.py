...
8. 如果你已经获得了足够回答用户问题的信息（例如用户只是想“看一下 README”），
   请立即停止调用工具，直接使用已有数据生成 Evidence → Analysis → Conclusion 格式的回答。
   不要为了凑齐三层证据而去调用无关工具。
9. 高级分析工具（analyze_repository、predict_roadmap 等）是自包含的原子操作。
    一旦调用，就禁止再调用任何基础数据工具。
    因为高级工具内部已经完成了所有证据收集，直接使用其返回的结果即可。
"""